...
<div class="row">
					...
					<div class="row">
						...
						<div class="col-4">
							...
							<div class="form-group">
								...
								<input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ $nomination->training->title ?? old('title') }}" id="title" placeholder="Enter Training Title" autocomplete="off">
							...
							<div class="form-group">
								...
								<select class="form-control" name="major_id" id="major_id">
									...
									@foreach ($majors as $major)
										<option value="{{ $major->id }}" {{ $nomination->training->major->id == $major->id ? ' selected' : '' }}>{{ $major->name }}</option>
									@endforeach
						...
						<div class="col-4">
							<div class="form-group">
								...
								<input type="text" name="vendor" class="form-control @error('vendor') is-invalid @enderror" placeholder="Enter Training Provider" value="{{ $nomination->vendor ?? old('vendor') }}">
						...
						<div class="col-3">
							<div class="form-group">
								...
								<input type="text" name="location" class="form-control @error('location') is-invalid @enderror" placeholder="Enter Training Location" value="{{ $nomination->location ?? old('location') }}">
						...
						<div class="col-3">
		                    ...
		                    <div class="form-group">
		                        ...
		                        <div class="input-group date" id="date-time-picker-7"
		                            ...
		                            <input type="text" class="form-control datetimepicker-input @error('start_date') is-invalid @enderror" data-target="#date-time-picker-7" name="start_date" id="start_date" placeholder="Enter Start Date" value="{{ $nomination->start_date->format('m/d/Y h:i A') ?? old('start_date') }}">
		                ...
		                <div class="col-3">
		                    ...
		                    <div class="form-group">
		                        ...
		                        <div class="input-group date" id="date-time-picker-8"
		                             data-target-input="nearest">
		                            <input type="text" class="form-control datetimepicker-input @error('end_date') is-invalid @enderror" data-target="#date-time-picker-8" name="end_date" id="end_date" placeholder="Enter End Date" value="{{ $nomination->end_date->format('m/d/Y h:i A') ?? old('end_date') }}">
		                ...
		                <div class="col-3">
							<div class="form-group">
								...
								<input type="text" name="qualification_id" class="form-control" value="{{ $nomination->qualification->name ?? old('qualification_id') }}" id="qualification_id" readonly>
							...
							<div class="form-group">
								...
								<select class="form-control @error('sponsor') is-invalid @enderror" name="sponsor" id="sponsor">
									...
									@foreach ($training->sponsors()['sponsors'] as $key => $sponsor)
										<option value="{{ $key }}" {{ $nomination->sponsor == $key ? '  selected' : '' }}>{{ $sponsor }}</option>
									@endforeach
								</select>
							...
							<div class="form-group">
								...
								<select class="form-control @error('resident') is-invalid @enderror" name="resident" id="resident">
									...
									@foreach ($training->sponsors()['types'] as $key => $sponsor)
										<option value="{{ $key }}" {{ $nomination->resident == $key ? ' selected' : '' }}>{{ $sponsor }}</option>
									@endforeach
								</select>
					...
					<div class="row mt-3">
						...
						<div class="col-12 mt-5">
							<div class="btn-group" role="group" aria-label="Basic example">
			                    <a href="{{ route('hr.nominations') }}" class="btn btn-sm btn-danger"><i class="icon icon-settings icon-lg mr-3"></i> {{ strtoupper('Cancel') }}</a>
	...
	<div class="col-4">
		<div class="dt-card">
			<div class="dt-card__body">
				<h3 class="mb-5">Nominated Staffs</h3>
				<ul class="list-group">
...
@section('scripts')
	...
	<script>
		...
		var start = "{{ $nomination->start_date }}";
		var end = "{{ $nomination->end_date }}";
		var current = "{{ $nomination->course_id }}";
	</script>
